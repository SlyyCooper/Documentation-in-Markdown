...
- [How to Replay and Branch from Prior States¶](#how-to-replay-and-branch-from-prior-states)
  - [Setup¶](#setup)
    - [SDK initialization¶](#sdk-initialization)
  - [Replay a state¶](#replay-a-state)
    - [Initial invocation¶](#initial-invocation)
  - [Branch off from previous state¶](#branch-off-from-previous-state)
  - [Comments](#comments)
...
How-to Guides
    ...
    * LangGraph Platform  LangGraph Platform 
      ...
      * Human-in-the-loop  Human-in-the-loop 
        ...
        * How to Replay and Branch from Prior States  How to Replay and Branch from Prior States  Table of contents 
          * Setup 
            * SDK initialization 
          * Replay a state 
            * Initial invocation 
...
With LangGraph Cloud you have the ability to return to any of your prior
states and either re-run the graph to reproduce issues noticed during testing,
or branch out in a different way from what was originally done in the prior
states. In this guide we will show a quick example of how to rerun past states
and how to branch off from previous states as well.
...
Before replaying a state - we need to create states to replay from! In order
to do this, let's invoke our graph with a simple message:
...
Now let's get our list of states, and invoke from the third state (right
before the tool get called):
...
To rerun from a state, we need first issue an empty update to the thread
state. Then we need to pass in the resulting `checkpoint_id` as follows:
...
As we can see, the graph restarted from the tool node with the same input as
our original graph run.
...
Let's update the state to change the input to the tool
...
Now we can rerun our graph with this new config, starting from the
`new_state`, which is a branch of our `state_to_replay`:
...
As we can see, the search query changed from San Francisco to SF, just as we
had hoped!
